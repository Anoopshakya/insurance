...
test('partner policy review requires approval, preserves history and rejects stale decisions',async()=>{const db=new PGlite();try{
...
await assert.rejects(create('other'),/your converted leads/);await assert.rejects(create('owner',{...details,tenureMonths:0}),/valid policy details/);assert.equal((await db.query('select count(*)::int n from customers')).rows[0].n,0);
