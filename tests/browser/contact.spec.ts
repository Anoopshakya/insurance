...
test("contact form validates, preserves failed submissions and confirms saved messages", async ({page,request}) => {
 ...
 await form.getByLabel('Full Name').fill('Contact Test');await form.getByLabel('Mobile Number').fill('9876543210');await form.getByLabel('Email Address').fill('contact@example.com');await form.getByLabel('State',{exact:true}).selectOption('Delhi');await form.getByLabel('Your Message').fill('Please help me understand my policy.');
