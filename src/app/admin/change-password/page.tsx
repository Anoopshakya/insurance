...
export default function ChangePasswordPage() {
  ...
  return <div className="dashboard-page"><div className="page-heading"><div><p className="admin-eyebrow">Account security</p><h1>Change password</h1><p>Keep your MagikPolicy administrator account protected.</p></div></div><section className="panel password-card"><span className="login-icon"><Icon name="shield"/></span><h2>Update your password</h2><p>Use at least 12 characters with a mix of letters, numbers, and symbols.</p><form onSubmit={submit}><label className="mp-label">Current password<input className="mp-control" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} autoComplete="current-password" required /></label><label className="mp-label">New password<input className="mp-control" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} autoComplete="new-password" required minLength={12}/></label><label className="mp-label">Confirm new password<input className="mp-control" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} autoComplete="new-password" required minLength={12}/></label>{message && <p className={message.type === "error" ? "form-error" : "form-success"} role="status">{message.text}</p>}<button className="primary-button" disabled={loading}>{loading ? "Updating…" : "Change password"}</button></form></section></div>;
}
