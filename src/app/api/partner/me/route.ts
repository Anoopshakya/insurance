...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db.from("agents").select("id,agent_code,status,kyc_status,joining_date,partner_type,region,users!inner(full_name,phone)").eq("user_id", decoded.uid).single();
