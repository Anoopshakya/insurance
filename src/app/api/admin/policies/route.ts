...
export async function GET(request: NextRequest) {
  ...
  const [policyResult, customerResult, agentResult, sectorResult, productResult, typeResult, insurerResult, planResult] = await Promise.all([
    db.from("policies").select("id,policy_number,customer_id,agent_id,category_id,product_id,product_type_id,insurer_id,plan_id,premium,coverage,start_date,expiry_date,tenure_months,business_type,status,policy_document_path,review_status,review_note,reviewed_at,created_at,updated_at,customer:customers!customer_id(id,name,contact,email),agent:agents!agent_id(id,agent_code,users:users!agents_user_id_fkey(full_name)),sector:categories!category_id(id,name),product:products!product_id(id,name,category:categories!category_id(id,name)),productType:product_types!product_type_id(id,name,category_id),insurer:insurers!insurer_id(id,name),plan:plans!plan_id(id,name)").order("created_at", { ascending: false }).limit(2000),
