...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db.from("agents").select("id,agent_code,partner_type,region,status,kyc_status,joining_date,created_at,users!inner(full_name,phone,email)").order("created_at", { ascending: false }).limit(1000);
