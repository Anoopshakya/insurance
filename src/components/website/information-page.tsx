...
export function InformationPage({ page, article = false }: { page: PageContent; article?: boolean }) {
  return <div className="public-site editorial-page"><main className="public-inner">
    ...
    <div className="editorial-grid"><div className="editorial-sections">
      {page.slug === "blog" && <div className="editorial-card-grid">{articles.map(item => <article key={item.slug}><span className="editorial-product-tag">{item.eyebrow}</span><h2>{item.title}</h2><p>{item.intro}</p><Link href={"/blog/" + item.slug}>Read guide ?</Link></article>)}</div>}
      ...
      {page.sections.map(section => <section className="editorial-section" key={section.title}><h2>{section.title}</h2>{section.paragraphs?.map(text => <p key={text}>{text}</p>)}{section.bullets && <ul>{section.bullets.map(text => <li key={text}>{text}</li>)}</ul>}{section.links && <div className="editorial-links">{section.links.map(link => <Link key={link.href} href={link.href}>{link.label}</Link>)}</div>}</section>)}
    ...
    </div><aside className="editorial-aside"><h2>Find your next step</h2><p>Explore a product or ask the team to contact you about your insurance needs.</p><Link href="/products">Explore insurance <ArrowRight size={16} /></Link><Link href="/contact">Contact the team <ArrowRight size={16} /></Link><a className="site-gradient" href="#get-quotation">Get quotation <ArrowRight size={16} /></a></aside></div>
  </main></div>;
}
