...
export default function QuotationForm({ pathname, initialProduct, sectionId = "get-quotation" }: { pathname: string; initialProduct?: string; sectionId?: string }) {
  ...
  <section id={sectionId} className="quotation-section" aria-labelledby={id + "-title"}>
    ...
    <form className="quotation-form" onSubmit={submit} noValidate aria-busy={busy}>
      ...
      <label htmlFor={id + "-phone"}>Contact Number *<span className="quotation-phone"><span className="quotation-country" aria-label="Country code India">+91</span><input id={id + "-phone"} name="mobile" type="tel" inputMode="tel" autoComplete="tel" placeholder="Enter your mobile number" maxLength={18} required disabled={busy} /></span></label></div>
      <label htmlFor={id + "-email"}>Email Address (Optional)<span className="quotation-icon-field"><Mail size={19} aria-hidden="true" /><input id={id + "-email"} name="email" type="email" autoComplete="email" placeholder="Enter your email address" maxLength={254} disabled={busy} /></span></label>
      ...
      <button className="quotation-submit" type="submit" disabled={busy}>{busy ? "Sending request..." : "Get Quote"}<ArrowRight size={18} aria-hidden="true" /></button>
