...
export function CommissionSlabEditorV2({ slabId }: { slabId?: string }) {
  ...
  return <div className="slab-editor">
    ...
    <section className="slab-basics">
      <label className="mp-label">Slab Name<input className="mp-control" value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Fresh Motor Slab" /></label>
      <label className="mp-label">Business Type<select className="mp-control" value={business} onChange={(event) => setBusiness(event.target.value as BusinessType)}><option value="fresh">Fresh</option><option value="port">Port</option><option value="renew">Renew</option></select><small>Define commission slabs for {business} business.</small></label>
      <label className="mp-label">Commission Category *<select className="mp-control" value={commissionCategory} onChange={(event) => { const id = event.target.value; setCommissionCategory(id); setSector(commissionCategories.find((item) => item.id === id)?.category_id || ""); }}><option value="">Select commission category</option>{commissionCategories.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.sector.name} / {item.type.name}</option>)}</select></label>
    </section>
    <section className="slab-matrix">
      ...
      {([['high', 'High Commission', '↗'], ['average', 'Avg. Commission', '▥'], ['low', 'Low Commission', '↘']] as const).map(([key, label, icon]) => <div className={`slab-matrix-row ${key}`} key={key}><div><b>{icon}</b><strong>{label}</strong></div>{ranges.map((range, index) => <label className="mp-label" key={index}><input className="mp-control" type="number" min="0" max="100" value={range[key]} onChange={(event) => changeRange(setRanges, index, key, Number(event.target.value))} /><span>%</span></label>)}</div>)}
    </section>
    ...
    {modalOpen && <div className="slab-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setModalOpen(false)}>
      <section className="slab-modal" role="dialog" aria-modal="true" aria-labelledby="slab-modal-title">
        ...
        <div className="slab-modal-details">
          <label className="mp-label">Business Type *<select className="mp-control" value={draftBusiness} onChange={(event) => setDraftBusiness(event.target.value as BusinessType)}><option value="fresh">Fresh</option><option value="port">Port</option><option value="renew">Renew</option></select></label>
          <label className="mp-label">Commission Category *<select className="mp-control" value={draftCommissionCategory} onChange={(event) => { const id = event.target.value; setDraftCommissionCategory(id); setDraftSector(commissionCategories.find((item) => item.id === id)?.category_id || ""); }}><option value="">Select commission category</option>{commissionCategories.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.sector.name} / {item.type.name}</option>)}</select></label>
          <label className="mp-label">Slab Name *<input className="mp-control" value={draftName} onChange={(event) => setDraftName(event.target.value)} placeholder="Fresh Motor Slab" /></label>
        </div>
        ...
        <div className="slab-range-table">
          ...
          {draftRanges.map((range, index) => <div className="slab-range-row" key={index}>
            ...
            <div className="range-pair"><span>From</span><input className="mp-control" value={index === 0 ? 0 : (draftRanges[index - 1].max || 0) + 1} readOnly /><span>To</span><input className="mp-control" type="number" min="1" value={range.max ?? ""} onChange={(event) => changeRange(setDraftRanges, index, "max", event.target.value ? Number(event.target.value) : null)} /></div>
            {(['high', 'average', 'low'] as const).map((key) => <label className="percent-input mp-label" key={key}><input className="mp-control" type="number" min="0" max="100" value={range[key]} onChange={(event) => changeRange(setDraftRanges, index, key, Number(event.target.value))} /><span>%</span></label>)}
