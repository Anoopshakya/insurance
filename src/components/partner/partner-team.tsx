      ...
      {!hasTeam ? <section className="pt-empty-hero"><div className="pt-empty-art"><img src="/brand/partner-team-empty.png" width={800} height={800} alt="Partners building a growing professional network together" /></div><div className="pt-empty-copy"><span className="pt-icon"><Users /></span><h2>Your Team is Empty for Now</h2><p>Start building your network by inviting new partners. They will complete registration and the approval process to start their partner journey.</p><button className="pt-primary" onClick={openInvite} disabled={!data.canInvite}><UserPlus />Invite Your First Partner<ArrowRight /></button><Steps /></div></section> :
        <section className="pt-card pt-team-table" aria-busy={loading}>
          ...
          <div className="pt-filters">
            <label className="mp-label">Date Range<select className="mp-control" value={period} onChange={event => { setPeriod(event.target.value); setPage(1); }}><option value="all">All time</option><option value="month">Joined this month</option><option value="year">Joined this year</option></select></label>
            <label className="mp-label">Sort by<select className="mp-control" value={sort} onChange={event => { setSort(event.target.value); setPage(1); }}><option value="newest">Newest first</option><option value="business">Higher Business</option><option value="commission">Higher Commission</option><option value="name">Name A–Z</option></select></label>
            <label className="mp-label">Status<select className="mp-control" aria-label="Status" value={status} onChange={event => { setStatus(event.target.value); setPage(1); }}><option value="all">All</option>{["active", "approved", "draft", "submitted", "under_review", "suspended", "rejected", "deactivated"].map(value => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label>
            <label className="mp-label">Team level<select className="mp-control" value={level} onChange={event => { setLevel(event.target.value); setPage(1); }}><option value="0">All levels</option><option value="1">Direct partners</option><option value="2">Level 2</option><option value="3">Level 3</option></select></label>
            <label className="pt-search mp-label"><span className="pt-sr-only">Search by name, code or mobile</span><Search /><input className="mp-control" value={search} onChange={event => setSearch(event.target.value)} placeholder="Search by name, code or mobile..." /></label><button onClick={reset}>Reset</button>
          </div>
      ...
      <div className={`pt-invite-grid ${tab === "details" ? "pt-invite-details" : ""}`}><section className="pt-invite-main"><h2>Invite a Partner</h2><p className="pt-invite-intro">{tab === "details" ? "Add partner details and prepare an invitation to join your team." : "Share your invite link or QR code to onboard new partners under your team."}</p>
        ...
        <div role="tabpanel" id="pt-invite-panel" aria-labelledby={tab === "link" ? "pt-link-tab" : "pt-details-tab"}>
          {tab === "link" ? <><label className="mp-label">Your Referral Code<div className="pt-copy-field"><input className="mp-control" aria-label="Your Referral Code" readOnly value={data.code} onFocus={event => event.currentTarget.select()} /><button aria-label="Copy referral code" onClick={() => void copy(data.code, "Referral code")}><Copy />Copy</button></div></label><p className="pt-field-help">Share this code with new partners along with your invitation link.</p><label className="mp-label">Your Invitation Link<div className="pt-copy-field"><input className="mp-control" aria-label="Your Invitation Link" readOnly value={url} onFocus={event => event.currentTarget.select()} /><button aria-label="Copy invitation link" onClick={() => void copy(url, "Invite link")}><Copy />Copy</button></div></label><p className="pt-field-help">Share this link via WhatsApp, email or any other channel.</p><h3>Quick Share</h3><div className="pt-quick-share"><a href={"https://wa.me/?text=" + encodeURIComponent("Join my MagikPolicy partner team: " + url)} target="_blank" rel="noopener noreferrer"><span className="pt-whatsapp"><MessageCircle /></span>Share on<br />WhatsApp</a><button onClick={() => void copy(url, "Invite link")}><span><Link2 /></span>Copy Link</button></div></> : <form className="pt-details-form" onSubmit={event => {
            ...
            if (!/^\d+$/.test(localPhone) || (inviteCountry === "91" ? !/^[6-9]\d{9}$/.test(localPhone) : !/^\d{10}$/.test(localPhone)) || (inviteCountry + localPhone).length > 15) { setNotice("Enter a valid mobile number without the country code."); return; }
            ...
            <div className="pt-details-fields">
              <label className="mp-label"><span>Full Name <b className="pt-required">*</b></span><div className="pt-input-icon mp-input-group"><UserRound aria-hidden="true" /><input className="mp-control" aria-label="Full Name" required minLength={2} value={inviteName} onChange={event => setInviteName(event.target.value)} maxLength={100} placeholder="Enter full name" autoComplete="name" /></div></label>
              <label className="mp-label"><span>Mobile Number <b className="pt-required">*</b></span><div className="pt-phone-field mp-input-group mp-phone-group"><select className="mp-control" aria-label="Country calling code" value={inviteCountry} onChange={event => setInviteCountry(event.target.value)}><option value="91">+91</option><option value="1">+1</option><option value="44">+44</option><option value="61">+61</option><option value="971">+971</option><option value="65">+65</option></select><Phone aria-hidden="true" /><input className="mp-control" aria-label="Mobile Number" required type="tel" inputMode="numeric" value={invitePhone} onChange={event => setInvitePhone(event.target.value)} placeholder="Enter mobile number" autoComplete="tel-national"  maxLength={10} minLength={10} pattern="[0-9]{10}" onInput={event => { event.currentTarget.value = event.currentTarget.value.replace(/\D/g, "").slice(0, 10); }} /></div></label>
            </div>
            <label className="mp-label">Personal Message (Optional)<div className="pt-input-icon pt-message-field mp-input-group"><MessageCircle aria-hidden="true" /><textarea className="mp-control" aria-label="Personal Message (Optional)" value={inviteMessage} onChange={event => setInviteMessage(event.target.value)} maxLength={1000} rows={4} /></div></label>
            ...
            <div className="pt-details-actions"><button type="button" onClick={() => setModal(null)}>Cancel</button><button className="pt-primary mp-form-action" type="submit" disabled={!viaWhatsApp || !data.canInvite}><Send aria-hidden="true" />Send Invitation</button></div>
          </form>}
