...
import { ArrowRight, BookOpen, Check, ChevronLeft, ChevronRight, Copy, Download, FileCheck2, Link2, MessageCircle, MoreVertical, Plus, Search, Share2, UserPlus, Users, X } from "lucide-react";
...
export function PartnerTeam() {
  ...
  return <div className="pt-page">
    ...
    {modal === "invite" && data && <Dialog title="Invite a Partner" wide close={() => setModal(null)}>
      <div className="pt-invite-grid"><section className="pt-invite-main"><h2>Invite a Partner</h2><p className="pt-invite-intro">Share your invite link or QR code to onboard new partners under your team.</p>
        ...
        <div role="tabpanel" id="pt-invite-panel" aria-labelledby={tab === "link" ? "pt-link-tab" : "pt-details-tab"}>
          {tab === "link" ? <><label>Your Referral Code<div className="pt-copy-field"><input aria-label="Your Referral Code" readOnly value={data.code} onFocus={event => event.currentTarget.select()} /><button aria-label="Copy referral code" onClick={() => void copy(data.code, "Referral code")}><Copy />Copy</button></div></label><p className="pt-field-help">Share this code with new partners along with your invitation link.</p><label>Your Invitation Link<div className="pt-copy-field"><input aria-label="Your Invitation Link" readOnly value={url} onFocus={event => event.currentTarget.select()} /><button aria-label="Copy invitation link" onClick={() => void copy(url, "Invite link")}><Copy />Copy</button></div></label><p className="pt-field-help">Share this link via WhatsApp, email or any other channel.</p><h3>Quick Share</h3><div className="pt-quick-share"><a href={"https://wa.me/?text=" + encodeURIComponent("Join my MagikPolicy partner team: " + url)} target="_blank" rel="noopener noreferrer"><span className="pt-whatsapp"><MessageCircle /></span>Share on<br />WhatsApp</a><button onClick={() => void copy(url, "Invite link")}><span><Link2 /></span>Copy Link</button></div></> : <form className="pt-details-form" onSubmit={event => { event.preventDefault(); let phone = invitePhone.replace(/\D/g, ""); if (phone.length === 10) phone = "91" + phone; if (!/^\d{10,15}$/.test(phone)) { setNotice("Enter a valid mobile number, including country code for international numbers."); return; } window.open("https://wa.me/" + phone + "?text=" + encodeURIComponent(`Hi ${inviteName.trim()}, join my MagikPolicy partner team: ${url}`), "_blank", "noopener,noreferrer"); }}><label>Partner name<input required value={inviteName} onChange={event => setInviteName(event.target.value)} maxLength={100} placeholder="Enter their full name" /></label><label>Mobile number<input required type="tel" value={invitePhone} onChange={event => setInvitePhone(event.target.value)} maxLength={20} placeholder="e.g. 9876543210" /></label><p>Opens a personalised WhatsApp invitation for you to review and send. They join your team when they register using your link.</p><button className="pt-primary" type="submit"><MessageCircle />Continue to WhatsApp</button></form>}
