...
type Lead = { id:string; name:string; contact:string|null; status:string; updated_at:string; product_sector:{name:string}|null; product_type:{name:string}|null };
export function ConvertedPolicyLeads({rows}:{rows:Lead[]}) {
 ...
 <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",textAlign:"left"}}><thead><tr>{["Name","Mobile","Product","Last updated","Status"].map(label=><th key={label} style={{padding:14}}>{label}</th>)}</tr></thead><tbody>{visible.map(row=><tr key={row.id}><td style={{padding:14}}>{row.name}</td><td style={{padding:14}}>{row.contact?<a href={`tel:${row.contact}`}>{row.contact}</a>:"Not provided"}</td><td style={{padding:14}}>{row.product_type?.name||row.product_sector?.name||"Not specified"}</td><td style={{padding:14}}>{new Date(row.updated_at).toLocaleDateString("en-IN")}</td><td style={{padding:14}}><span className="pp-status">Converted</span></td></tr>)}</tbody></table></div>
