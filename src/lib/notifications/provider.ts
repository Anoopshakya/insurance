...
type WelcomeMessage = { mobile?: string; email?: string; fullName: string; agentCode: string; inviteUrl: string };

export type DeliveryResult = { channel: "email" | "sms" | "whatsapp"; status: "sent" | "queued" | "failed"; detail?: string };
...
async function deliver(channel: "email" | "sms" | "whatsapp", input: WelcomeMessage): Promise<DeliveryResult> {
  const url = channel === "email" ? process.env.EMAIL_WEBHOOK_URL : channel === "sms" ? process.env.SMS_WEBHOOK_URL : process.env.WHATSAPP_WEBHOOK_URL;
...
export async function sendPartnerWelcome(input: WelcomeMessage) {
  const channels: Array<"email" | "sms" | "whatsapp"> = [...(input.email ? ["email" as const] : []), ...(input.mobile ? ["sms" as const, "whatsapp" as const] : [])];
