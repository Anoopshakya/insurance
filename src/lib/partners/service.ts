...
export async function createPartner(raw: PartnerInput, actorId: string) {
  ...
  try {
    ...
    const { data: agent, error: agentError } = await db.from("agents").insert({ user_id: firebaseUser.uid, agent_code: agentCode, sponsor_id: sponsorId, agent_type: "partner", partner_type: input.partnerType || "standard", source: "admin_invite", region: input.region || null, status: "invited", kyc_status: "not_started", performance_level: "starter", created_at: now, updated_at: now }).select().single();
    ...
    const delivery = await sendPartnerWelcome({ mobile: input.mobile || undefined, email: input.email || undefined, fullName: input.fullName, agentCode, inviteUrl });
